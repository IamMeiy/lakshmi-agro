...
class InvoiceController extends Controller
{
    ...
    public function finalPrice(Request $request){
        $request->validate([
            ...
            'quantity.*.required' => 'Quantity at position :position is required',
            'quantity.*.numeric' => 'Quantity at position :position must be a number',
            'quantity.*.min' => 'Quantity at position :position must be at least 1',
        ]);
    
        if (!$request->has("products") || empty($request->products)) {
            return response()->json(['status' => 'error', 'message' => 'Please add at least one product.']);
        }
        ...
        foreach ($request->products as $index => $product) {
            ...
            if ($request->quantity[$index] > $variant->stock_quantity) {
                return response()->json([
                    'status' => 'error',
                    'message' => "Insufficient stock for '{$variant->product->name} - {$variant->quantity}g'. Only {$variant->stock_quantity} left."
                ]);
            }
            ...
            try {
                ...
                foreach($request->products as $index => $product){
                    ...
                    if($product){
                        ...
                        $product->stock_quantity = $product->stock_quantity - $request->quantity[$index];
                        $product->save();
                    }
                }
